...
- `wy` 升级为统一总菜单，集中提供状态、检查、代理接入、重启和原始 WireGuard 状态。
- `status` 改为人类可读摘要，新增 `status-raw` 保留原始输出。
- 安装和启动前检测其他 WireGuard 接口是否复用相同 WARP 身份，发现冲突时拒绝并保留旧服务。
- README 精简为快速入口，原理、安全边界、高级参数和故障排查拆分到 `docs/`。
