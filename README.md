...
**v1.1.1 新增可选代理接入**：支持 Mihomo / sing-box 的单文件、直连出口服务端配置。主动执行接入命令后才修改指定代理，不影响普通安装行为。
...
`wy` 是 `warp-yundan` 的安全短命令；原长命令仍可使用。菜单会让你选择现有代理、服务名、`hybrid` / `all` 模式，并在真正修改前要求确认；存在多个运行实例时会列出供选择。选择“只预检”可先验证配置而不重启服务。
